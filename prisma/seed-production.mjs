...
async function main() {
  ...
  console.log("⚠️  This will add data to your production database!");
  console.log("");
  ...
  if (!isSupabase) {
    console.error("❌ This script should only run against Supabase!");
    console.error(
  ...
  try {
    ...
    // Create 7 sellers
    console.log("👨‍🌾 Creating 7 production sellers...");
    ...
    console.log("✅ Created production user");
    ...
    console.log("");
    console.log("⚠️  Remember to change these passwords after first login!");
    console.log("═══════════════════════════════════════════════════");
  } catch (error) {
    console.error("");
    ...
    if (error.code === "P2002") {
      console.error("⚠️  Data already exists in the database.");
      ...
      console.error(
        "  2. Or use: npm run db:migrate:deploy (⚠️ RESETS DATABASE!)",
      );
    } else {
